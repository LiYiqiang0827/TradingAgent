...
# ====== 业务常量 ======
CTRL_KEY = "cn_daily_basic"  # tbl_basic_ctrl key
SERVICE_NAME = "service_daily_basic"
...
def main():
    ...
    if args.trade_date:
        # 单日模式
        n = down.update_daily_basic(start_date=args.trade_date, end_date=args.trade_date)
    else:
        # 增量模式(默认从 ctrl 取)
        from core.offline_db_client import get_conn, get_ctrl
        conn = get_conn("basic")
        sd, ed, desc = resolve_date_range(conn, CTRL_KEY, args, default_start="20150105")
        n = down.update_daily_basic(start_date=sd, end_date=ed)
