"""
service_margin_detail.py - 融资融券交易明细更新
...
# ====== 业务常量 ======
CTRL_KEY = "cn_margin_detail"  # tbl_basic_ctrl key
SERVICE_NAME = "service_margin_detail"
...
def main():
    ...
    if args.trade_date:
        # 单日模式
        n = down.update_margin_detail(start_date=args.trade_date, end_date=args.trade_date)
    else:
        # 增量模式(默认从 ctrl 取)
        from core.offline_db_client import get_conn, get_ctrl
        conn = get_conn("basic")
        sd, ed, desc = resolve_date_range(conn, CTRL_KEY, args)
        n = down.update_margin_detail(start_date=sd, end_date=ed)
